...
export default function ProductsCatalog({ initialProducts }: { initialProducts?: readonly Product[] }) {
  ...
  const filteredAndSortedProducts = useMemo(() => {
    ...
    if (activeCategory === "all") {
      ...
      result = catalogProducts.filter((p) => p.category === mainCat && p.name.startsWith(series));
    } else {
      result = catalogProducts.filter((p) => p.category === activeCategory);
    }
