...
export function WheelchairFinderClient({
  candidates,
  catalogError = false,
}: Props) {
  const { assessment, step, update, next, back, reset, result } =
    useWheelchairAssessment(candidates);
  ...
  const changeMobilityType = (mobilityType: MobilityType) => {
    const allowedPriorities = assessment.use.priorities.filter((priority) =>
      mobilityType === "powered"
        ? priority !== "self-propulsion"
        : priority !== "range",
    );
    update({
      mobilityType,
      use: {
        priorities:
          allowedPriorities.length > 0 ? allowedPriorities : ["fit"],
      },
    });
  };
  ...
  const validateUse = () => {
    ...
    if (
      ...
      (assessment.use.dailyRangeKm < 1 || assessment.use.dailyRangeKm > 100)
    ) {
  ...
  const priorities: readonly Priority[] =
    assessment.mobilityType === "powered"
      ? ["fit", "portability", "range", "rough-terrain", "roominess"]
      : [
          "fit",
          "portability",
          "self-propulsion",
        ...
        {step === 1 && (
          <section className="card p-6 sm:p-8" aria-labelledby="basics-heading">
            ...
            <fieldset className="mt-6">
              <legend className="font-medium">Wheelchair type</legend>
              <div className="mt-3 grid gap-3 sm:grid-cols-2">
                {([
                  ["powered", "Powered wheelchair", "Motorized mobility with battery-powered driving."],
                  ["manual", "Manual wheelchair", "Self-propelled or caregiver-assisted mobility."],
                ] as const).map(([value, label, description]) => (
                  <label
                    key={value}
                    className="rounded-lg border border-stone bg-pure-white p-4"
                  >
                    <input
                      type="radio"
                      name="mobilityType"
                      value={value}
                      checked={assessment.mobilityType === value}
                      onChange={() => changeMobilityType(value)}
                    />
                    <span className="ml-2 font-medium">{label}</span>
                    <span className="mt-1 block pl-6 text-sm text-warm-charcoal">
                      {description}
                    </span>
                  </label>
                ))}
