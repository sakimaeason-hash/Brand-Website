...
import type { PublicProduct, PublicPromotion, PublicStory } from "@/lib/content/repository";
...
export default function HomePage({
  ...
  featuredProducts?: readonly PublicProduct[];
  ...
  return (
    <div className="flex flex-col">
      ...
      {promotion && (
        <section className="border-b border-[#E8DDD4] bg-[#3D3330] text-white">
          <div className="mx-auto flex max-w-7xl flex-col items-center gap-3 px-4 py-4 text-center sm:flex-row sm:justify-center sm:gap-5 sm:px-6 lg:px-8">
            <Badge className="bg-[#F5A623] text-[#2D2D2D]">{promotion.label || "Limited-time offer"}</Badge>
            <p className="text-sm font-medium sm:text-base">{promotion.name} on {promotion.productName}</p>
            <Link href={`/products#${promotion.productId}`} className="text-sm font-semibold text-[#E8D5C4] underline underline-offset-4 hover:text-white">Shop offer</Link>
          </div>
        </section>
      )}
                    ...
                    <CardContent className="p-6">
                      <div className="flex items-center gap-3 mb-4">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-[#5C534E]">{testimonial.age ? `Age ${testimonial.age}` : testimonial.location || "GoldSeason customer"}</p>
                        </div>
                      </div>
