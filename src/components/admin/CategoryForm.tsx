...
export function CategoryForm({ initialData }: { initialData?: CategoryFormData }) {
  ...
  async function submit(event: React.FormEvent) {
    ...
    setBusy(true);
    setMessage("");
    const payload = { ...form, sortOrder: Number(form.sortOrder), description: form.description || null, fields: form.fields.filter((field) => !field.isProtected && !field.semanticKey && field.status !== "ARCHIVED").map((field, sortOrder) => ({ ...field, sortOrder, isProtected: false, semanticKey: null })) };
    try {
      const response = await fetch(initialData ? `/api/admin/product-categories/${initialData.id}` : "/api/admin/product-categories", { method: initialData ? "PATCH" : "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(initialData ? { ...payload, updatedAt: initialData.updatedAt } : payload) });
